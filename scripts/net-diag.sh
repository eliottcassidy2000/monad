...
probe() {
  ...
  dl="$(curl -sS -m20 -o /dev/null -w '%{size_download}' 'https://speed.cloudflare.com/__down?bytes=1000000' 2>/dev/null || echo 0)"
  ...
  local verdict="ok"
  if [ "$small_ok" = yes ] && [ "$large_ok" = no ]; then verdict="BLACK_HOLE_large_tcp"
...
{"host":"$HOST","gateway":"$gw","routing_via_v1410":$([ $via_v1410 = yes ] && echo true || echo false),
 "ping_8888":"$ping_ok","small_tcp":"$small_ok","large_tcp_1mb":"$large_ok",
 "path_mtu":$pmtu,"first_hops":"${hop:-}","verdict":"$verdict"}
EOF
  echo "{\"ts\":\"probe\",\"node\":\"$HOST\",\"source\":\"net-diag\",\"action\":\"probe\",\"result\":\"$verdict\",\"detail\":\"gw=$gw via_v1410=$via_v1410 ping=$ping_ok small=$small_ok large=$large_ok pmtu=$pmtu\"}" >> "$EVENTS"
  echo "[net-diag] $HOST: verdict=$verdict gw=$gw via_v1410=$via_v1410 ping=$ping_ok small_tcp=$small_ok large_tcp=$large_ok pmtu=$pmtu"
